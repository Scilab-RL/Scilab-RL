...
test_algos() {
  ...
  if ! xvfb-run -a /root/miniconda3/bin/python3 main.py env=$ENVS algorithm=$ALGOS +performance=smoke_test --multirun;
  then
    exit 1
  fi
}
...
test_envs() {
  ...
  # Don't have xvfb? install it with sudo apt-get install xvfb
  if ! xvfb-run -a /root/miniconda3/bin/python3 main.py algorithm=sac env=$ENVS +performance=smoke_test --multirun;
  then
    exit 1
  fi
}
