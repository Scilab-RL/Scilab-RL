...
def oo_evaluate_policy(
    ...
    :return: Mean reward per episode, std of reward per episode, success rate per object
        Returns ([float], [int], [float]) when ``return_episode_rewards`` is True, first
    ...
    tries_per_object = np.zeros(env.envs[0].n_objects + 1)
