...
class CLEANSACMC:
    ...
    def _create_actor_critic(self) -> None:
        ...
        self.mc_optimizer = torch.optim.Adam(self.mc_network.parameters(), lr=self.mc["learning_rate"])
    ...
    def train_mc(self, observations, next_observations, actions):
        ...
        fw_loss = -forward_normal.log_prob(next_observations).mean()
        w_prime_a_loss = -world_normal.log_prob(next_observations).mean()
        loss = fw_loss + w_prime_a_loss
